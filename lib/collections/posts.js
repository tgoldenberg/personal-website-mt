  ...
  if (Posts.find().count() === 0 ) {
    Posts.insert({
      ...
      paragraphs: [
        ...
        "Now to get setup with Stripe, you should first create an account at https://stripe.com/. There you’ll get a set of keys, both for testing and for when your website is ready to launch. Here we’ll be dealing with test mode. Once you have your publishable and secret keys, you’ll want to use the figaro gem to encrypt that information so that it isn’t passed into your git commits and Github. Check out figaro’s documentation here.",
        ...
        "First you will see the omniauth_callbacks controller file. This is to allow our users to register their Stripe accounts in order to be sellers. This is accomplished through the omniauth_stripe_connect gem. More information on the Stripe Connect options can be found here. Once your user passes through the authorization, they will become connected accounts on your Stripe dashboard.",
        "Next we can see that in our charges controller, we have made a few changes. Compare this to the Stripe example here. We have seperated the Stripe::Customer.create and Stripe::Charge.create actions into two seperate controller actions - create and complete. In the create action, we save our user’s information in the charge model of our database. Once it is time to charge our customer, we call the complete action with parameters for the destination and user token. If we look in our Stripe dashboard, we will be able to see the transaction transferring money to our seller, and an application fee ( defined in the charges#complete action) to us.",
        "Well, that’s about it for creating payments with Stripe. I hope you enjoyed this blog post. Please share your comments below. I am by no means an expert, and welcome corrections / improvements. Also, for the full repo, visit my Github site here. Happy coding!"
      ],
